...
def require_complete_unique_location_assignment(
        map_source: str,
        candidates_by_slot: list[tuple[str, list[str]]],
) -> None:
    """Require a distinct location template for every RANDOM slot."""
    location_to_slot: dict[str, int] = {}

    def assign(
            slot_index: int,
            visited_locations: set[str],
    ) -> bool:
        for location_id in candidates_by_slot[slot_index][1]:
            if location_id in visited_locations:
                continue

            visited_locations.add(location_id)
            previous_slot = location_to_slot.get(location_id)

            if previous_slot is None or assign(
                    previous_slot,
                    visited_locations,
            ):
                location_to_slot[location_id] = slot_index
                return True

        return False

    for slot_index in range(len(candidates_by_slot)):
        assign(slot_index, set())

    matched_slot_indexes = set(location_to_slot.values())

    if len(matched_slot_indexes) == len(candidates_by_slot):
        return

    unmatched_slot_ids = [
        slot_id
        for index, (slot_id, _) in enumerate(candidates_by_slot)
        if index not in matched_slot_indexes
    ]

    raise CatalogValidationError(
        f"{map_source}: RANDOM location slots cannot all be filled "
        f"with unique locations after reserving FIXED locations; "
        f"matched {len(matched_slot_indexes)} of "
        f"{len(candidates_by_slot)}, unmatched slots: "
        f"{unmatched_slot_ids}"
    )
...
def validate_catalog(
    ...
    for map_item in catalog["maps"]:
        ...
        fixed_location_ids = {
            ...
            and slot.get("fixedLocationTemplateId") is not None
        }
        random_location_candidates: list[
            tuple[str, list[str]]
        ] = []
        ...
        for slot in location_slots:
            ...
            if mode == "RANDOM":
                ...
                available_location_ids = [
                    ...
                    if location["templateId"] not in fixed_location_ids
                ]

                if not available_location_ids:
                    raise CatalogValidationError(
                        f"{slot_source}: all matching locations are "
                        f"reserved by FIXED slots"
                    )

                random_location_candidates.append(
                    (
                        slot["slotId"],
                        available_location_ids,
                    )
